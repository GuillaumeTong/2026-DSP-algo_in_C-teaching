...
\noindent \textbf{Objectifs du TP}
 \begin{itemize}
\item Apprendre à découper un programme en plusieurs fonctions ou procédures.
 \item Apprendre à manipuler les tableaux de caractères à une dimension
 \item Apprendre à manipuler les tableaux de caractères à deux dimensions
    \item Apprendre à utiliser les structures de contrôles itératives (boucles  \texttt{while})
 \end{itemize}
\section{Rappels : bibliothèque et compilation}
La bibliothèque d'entrée-sortie:
 \href{https://cedric.cnam.fr/~lamberta/enseignements/DSP/C/sources/inout.h}{inout.h} et   \href{https://cedric.cnam.fr/~lamberta/enseignements/DSP/C/sources/inout.c}{inout.c} mettez les dans un répertoire \texttt{tp4}. Pour cela vous pouvez
...
\end{lstlisting}
Après cet appel, \lstinline!t! est un tableau contenant tous les mots du fichier \lstinline!"toto.txt"!, et \lstinline!x[0]! contient la taille de ce tableau.
...
Vous pouvez télécharger un exemple de fichier \lstinline!"toto.txt"! avec la commande suivante :
...
\end{lstlisting}
  ...
  \begin{exo}[Manipulation des chaînes de caractères]
    Implantez les différentes fonctions du \href{https://cedric.cnam.fr/~lamberta/enseignements/DSP/C/sources/tp4_exo2.c}{squelette} fourni sur le site du cours :
    \begin{enumerate}
...
Lorsqu'on veut répéter une séquence d'instructions plusieurs fois, on utilise une \emph{boucle}. La boucle \verb|while| ressemble à un \lstinline!if! sans \lstinline!else!:
...
\item Puis on revient à l'étape 1. ci-dessus.
\end{enumerate}

Notez que si le corps de la boucle ne modifie par la valeur de \lstinline!cond! le programme va boucler indéfiniment.
\bigskip
...
On déclarera également un \verb|BOOL jouer| qui est initialisé à  \verb|TRUE|, et qui sera mis à  \verb|FALSE| quand le joueur aura gagné, pour pouvoir arrêter la partie.\\
 ...
 Implantez les différentes fonctions du \href{https://cedric.cnam.fr/~lamberta/enseignements/DSP/C/sources/pendu.c}{squelette} fourni sur le site du cours :
    \begin{enumerate}
